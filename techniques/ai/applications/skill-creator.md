...
> #### AI Annotation:
> 소프트웨어 개발의 엄밀함(테스트, 벤치마크, 반복 개선)을 코드 없이 스킬 작성에 적용했다.
...
> #### AI Annotation:
> capability uplift은 모델이 못 하는 걸 가능하게 하는 스킬, encoded preference는 이미 가능한 걸 팀 프로세스에 맞게 엮는 스킬.
...
> #### AI Annotation:
> 순차 실행 시 앞 테스트의 컨텍스트가 뒤 테스트에 묻어난다.
...
> #### AI Annotation:
> description이 너무 넓으면 엉뚱한 상황에서 트리거(false positive), 너무 좁으면 필요한 상황에서 안 트리거(false negative).
...
> #### AI Annotation:
> 현재 스킬은 "어떻게(how)"를 상세히 지시한다. 모델이 좋아지면 "무엇을(what)"만 기술해도 충분해질 것이다.
