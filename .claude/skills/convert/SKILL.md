...
**AI Annotation 중복 검토 (병합 시)**: 기존 knowledge 파일에 OA 단락을 추가·통합한 후, 그 Q&A의 기존 AI Annotation이 새 OA 단락으로 커버되는지 확인한다. 커버되면 자동 삭제, 일부만 중복이면 중복 부분만 삭제를 사용자에게 보고 후 적용한다.
