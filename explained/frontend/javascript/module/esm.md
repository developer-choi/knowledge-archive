...
ESM의 "한 번만 평가" 특성은 모듈 단위 싱글턴 패턴을 자연스럽게 가능하게 한다. 설정 객체나 캐시 저장소를 모듈 수준 변수로 두면 어디서 import해도 같은 인스턴스를 공유한다. 반면 모듈 수준의 side-effect(초기화 코드, 로그 등)는 단 한 번만 실행되므로, 테스트 환경에서 모듈 상태를 리셋하려면 Vitest의 `vi.resetModules()` 같은 명시적 리셋이 필요하다.
