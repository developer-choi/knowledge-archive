...
브라우저는 항상 사용자와 상호작용할 수 있어야 한다. JS 메인 스레드가 블로킹 상태에 빠지면 렌더링도, 클릭 처리도 모두 멈춘다.

---

## 본문

프론트엔드에서 블로킹을 피해야 하는 이유는 OA 없이 실무 원칙으로 정리한다.

브라우저의 메인 스레드는 JS 실행 + 렌더링 + 이벤트 처리를 모두 담당한다. 이 스레드가 오래 걸리는 JS 실행에 묶이면 나머지 두 작업이 차단된다.

```
메인 스레드가 하는 일
├── JS 실행 (이벤트 핸들러, 타이머 콜백 등)
├── 렌더링 (Style → Layout → Paint → Composite)
└── 사용자 이벤트 처리 (클릭, 스크롤, 키보드)

→ JS가 블로킹이면 렌더링과 이벤트 처리가 차단됨
```

가장 빈번한 예는 API 요청이다. `fetch()`를 블로킹 방식으로 처리하면(가령 동기 XHR) 응답을 받는 동안 UI 전체가 굳는다. `fetch()`를 비동기로 사용하면 응답 대기 중에도 사용자가 스크롤하고 클릭할 수 있다.

---

## 종합

프론트엔드에서 블로킹을 피하는 것은 UX 품질과 직결된다. Long Task(50ms 이상 메인 스레드를 점유하는 작업)는 Core Web Vitals의 INP(Interaction to Next Paint) 지표를 나쁘게 만든다. 무거운 연산은 `Web Worker`로, 렌더링에 직접 영향을 주는 애니메이션은 `requestAnimationFrame`으로 적절히 분배하는 것이 성능 최적화의 핵심이다.
