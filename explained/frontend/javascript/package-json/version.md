...
└─┬ next@14.0.0
  └── react@18.2.0
└── react@18.2.0   ← 직접 의존성
# react가 18.2.0으로 하나만 설치됨 → 정상

# 버전 충돌 상황
$ npm ls eslint
my-project@1.0.0
├── eslint@9.0.0                         ← 프로젝트 직접 의존
└─┬ @next/eslint-plugin-next@14.0.0
  └── eslint@8.0.0 deduped              ← deduped = 중복 제거됨
```

그래프를 보는 주된 이유:
1. **버전 충돌 확인**: 같은 패키지의 다른 버전이 중복 설치됐는가?
2. **의도한 대로 설치됐는가**: peerDependencies가 올바른 버전으로 설치됐는가?
3. **불필요한 의존성 탐지**: 직접 쓰지 않는데 설치된 것은 무엇인가?

---

## 종합

의존성 그래프는 `node_modules`라는 복잡한 패키지 숲을 탐색하는 지도다. Next.js 프로젝트의 `node_modules`를 열어보면 수백 개의 패키지가 있는데, 의존성 그래프 없이는 어떤 것이 어떤 것을 필요로 하는지 파악할 수 없다. 버전 충돌 에러(`Peer dependency conflict`)가 발생하면 `npm ls <패키지명>`으로 어느 패키지가 어떤 버전을 요구하는지 추적하는 것이 첫 번째 디버깅 단계다.
