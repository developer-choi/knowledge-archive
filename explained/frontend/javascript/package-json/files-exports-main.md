...
`.npmignore`가 있으면 `.gitignore`는 npm 배포에서 완전히 무시된다. 이 점이 함정이다 — 프로젝트에 `.gitignore`만 있다가 `.npmignore`를 추가하면, `.gitignore`에만 있던 제외 항목들이 갑자기 배포 패키지에 포함될 수 있다. `.npmignore`를 추가할 때는 `.gitignore`의 항목들을 같이 복사해두는 것이 안전하다. `files` 필드와 함께 쓰면 포지티브 방식(포함할 것 지정)이라 더 명확하다.
