...
여기서 "모킹은 테스트 범위를 좁힌다"와 "E2E는 넓은 범위를 테스트한다"가 충돌하는 것처럼 느껴질 수 있는데, 둘은 **서로 다른 축**이라 모순이 아니다.

- **깊이(트로피 레벨)**: 내 앱의 몇 개 층을 한꺼번에 관통하느냐 — unit(한 조각) → integration(여러 조각) → E2E(프론트+백 전체). E2E가 "넓다"는 건 이 뜻이고, 이 넓음이 확신(장점)과 원인 추적 어려움(단점)을 동시에 낳는다.
- **경계(제어권)**: 내가 통제하는 것 vs 남의 것. third-party 모킹이 잘라내는 건 이 경계 **바깥**이다.

third-party를 모킹으로 격리해도 그 테스트는 **여전히 내 앱 전체를 관통하는 E2E**다 — 트로피에서 아래로 내려가지 않는다. 모킹이 잘라낸 것은 "내 앱의 층(깊이)"이 아니라 "애초에 내 것도 아닌 외부(경계)"이기 때문이다. 즉 E2E에서의 모킹은 E2E의 넓음(확신)과 싸우는 게 아니라, 통제 불가능한 외부를 걷어내 그 넓은 테스트를 안정적으로 유지하는 장치다. (내 앱의 층까지 모킹해 버리면 그건 E2E의 존재 이유인 넓은 확신을 스스로 없애는 것 — 그래서 "모킹은 최대한 작게"는 '내 것은 실제로 두고 통제 불가 외부만 자른다'는 뜻이다.)
