...
(왜 정리가 필요한지 — 여러 테스트가 같은 가짜 하나를 공유해 호출 기록이 다음 테스트로 새는 문제 — 는 [overview.md](./overview.md)의 「같은 가짜 함수를 여러 테스트에서 재사용할 때, 왜 매 테스트마다 mock을 정리해줘야 하는가?」에서 다룬다. 여기서는 정리 함수 세 가지가 각각 무엇까지 되돌리는지에 집중한다.)
