...
Pages Router SSR에서만 보일러플레이트 제거(단일 HydrationBoundary)가 가능했던 것으로 보인다. App Router에서는 각 서버 컴포넌트에서 독립적으로 처리한다.
...
기본 설정으로는 성공한 쿼리만 dehydrate되고 클라이언트에서 다시 refetch하는 구조다.
