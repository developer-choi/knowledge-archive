...
영어 단어 "throughput"을 한국어로 옮길 때 "처리량"이 채택된 이유를 어원에서 풀어보면, 단순히 "지나간 양"이 아니라 **시스템이 일을 해서 내보낸 결과물의 양**이라는 뉘앙스가 살아 있다는 것이 보입니다. 단어 선택에 들어 있는 직관을 잡으면 throughput이라는 개념을 더 정확히 쓰게 됩니다.
...
- **through**: "통과하여, 거쳐서". 시스템의 내부 흐름을 지나간다는 방향성.
- **put**: "놓다, 투입하다, 출력하다". 결과물로 내보낸다는 동작.
- **through + put**: "거쳐서 내놓은 것". 즉 시스템에 들어가서 처리 과정을 통과한 뒤 밖으로 나온 양.

영어 원어가 "단순한 흐름"이 아니라 **"내부 처리 단계를 거친 결과"** 임을 강조하는 단어입니다. 그래서 throughput이라는 개념 자체가 "회선이 가진 물리적 폭(=bandwidth)"이 아니라 "시스템이 실제로 일을 끝내서 내보낸 양"을 가리키는 것.

라우터 입장에서 패킷 한 개가 "통과"하는 동안 일어나는 일을 보면 그 "처리"의 의미가 더 또렷합니다:

- 패킷이 들어오면 헤더 파싱(parsing) — 어디서 왔고 어디로 가는지 읽기.
- 라우팅 테이블 조회 — 다음 hop 결정.
- TTL 감소·체크섬 재계산 — 내용 검증 및 갱신.
- 출력 큐로 분배 — 어느 인터페이스로 내보낼지.
- 다시 패킷화 — 프레임으로 포장 후 송신.

이 모든 단계가 라우터 CPU·메모리·버퍼라는 **자원을 소비하는 일(work)**입니다. 그래서 "흐름"이 아니라 "처리"라는 단어가 어울립니다.
...
| 후보 번역 | 잡힌 의미 | 빠진 의미 |
|---|---|---|
| 전송량 | 보내진 양 | 시스템이 일을 했다는 뉘앙스가 약함 |
| 통과량 | 지나간 양 | 처리 단계를 거쳤다는 점이 안 보임 |
| **처리량** | 시스템이 일을 해서 내보낸 양 | (어원 그대로) |

"처리량"이 결국 채택된 이유는 단순한 "지나감"이 아니라 **장비가 자원을 써서 일을 끝낸 결과물**이라는 영어 어원의 뉘앙스를 가장 잘 살리기 때문입니다. 톨게이트 비유로 다시 보면:

- "전송량/통과량"으로 번역하면 → "차가 도로를 지나간 양". 톨게이트의 처리 능력이 안 보임.
- "처리량"으로 번역하면 → "톨게이트가 통행료를 받고 차를 통과시킨 결과의 양". 시스템의 일이 한 번에 들어옴.

브라우저 개발자 관점에서 이 직관이 닿는 곳: 큰 webpack 번들을 다운로드할 때 throughput이 낮은 게 회선 탓만은 아닐 수 있습니다 — 서버 nginx의 압축·전송 처리, CDN 엣지의 캐시 처리, 중간 라우터의 정책 처리, 클라이언트의 SSL 복호화까지 모두 "처리"의 단계입니다. 단어가 "처리량"이라 옮겨졌기 때문에 "어느 처리에서 막히고 있는가"라는 진단 질문이 자연스럽게 떠오릅니다.

오개념 예방: "처리량 = 단순히 흘러간 데이터 양"이라는 단순화는 단어가 가진 풍부한 의미를 잃어버립니다. throughput은 시스템이 "할 수 있는 일의 능력 × 실제로 한 일의 결과"의 결합이고, 한국어 "처리량"은 그 결합을 한 단어로 잘 담아낸 번역입니다.

이게 없으면 어떻게 되는가 — 단어가 "전송량"이었다면: 라우터·서버 CPU 한계가 throughput을 결정한다는 직관이 약해져, "회선만 늘리면 빨라진다"는 잘못된 추론이 더 흔했을 가능성이 큽니다. 단어 선택이 사고 패턴을 형성하는 사례.
