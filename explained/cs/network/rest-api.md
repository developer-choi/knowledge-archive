...
URL이 "무엇을"이라면, HTTP 메서드는 "어떻게"를 표현한다. 같은 `/users/42`라도 GET이냐 DELETE냐에 따라 완전히 다른 동작이 된다. OA는 4가지 메서드와 멱등성(idempotency) 개념을 함께 설명한다.
...
"개발자는 보통 HTTP를 사용해 RESTful API를 구현한다. HTTP 메서드는 서버에게 리소스에 대해 무엇을 해야 하는지 알려준다."
...
"클라이언트는 GET을 사용해 지정된 URL에 있는 리소스에 접근한다. GET 요청은 캐시할 수 있고, 서버가 전송 전에 데이터를 필터링하도록 파라미터를 보낼 수 있다."

**POST**
> Clients use POST to send data to the server. They include the data representation with the request.
> Sending the same POST request multiple times has the side effect of creating the same resource multiple times.

"클라이언트는 POST를 사용해 서버에 데이터를 보낸다. 같은 POST 요청을 여러 번 보내면 같은 리소스가 여러 번 생성되는 부작용이 있다."

- **side effect**: "부작용" — POST가 멱등하지 않다는 핵심 단어. 같은 요청 N번 = N개 생성.
...
"클라이언트는 PUT을 사용해 서버의 기존 리소스를 업데이트한다. POST와 달리 같은 PUT 요청을 여러 번 보내도 동일한 결과가 나온다."

- **gives the same result**: 멱등(idempotent)의 정의 그대로. 같은 요청 N번 = 항상 같은 상태.
...
"클라이언트는 DELETE 요청을 사용해 리소스를 제거한다. DELETE 요청은 서버 상태를 변경할 수 있다. 그러나 적절한 인증이 없으면 요청은 실패한다."

```
4가지 메서드 비교

메서드   동작        멱등성   Safe   사용 예
GET     조회        ✅ 멱등  ✅ Safe  GET /users/42
POST    생성        ❌ 비멱등 ❌      POST /users
PUT     전체 교체   ✅ 멱등  ❌      PUT /users/42
DELETE  삭제        ✅ 멱등  ❌      DELETE /users/42
```
...
POST와 PUT의 핵심 차이는 멱등성이다. `POST /orders`를 3번 호출하면 주문이 3개 생기지만, `PUT /orders/99`를 3번 호출하면 동일한 주문이 세 번 덮여쓰여 결과가 같다. 네트워크 오류로 재시도가 필요한 상황에서 멱등 메서드인지 아닌지가 재시도 안전성을 결정한다.
...
서버 응답의 첫 번째 구성요소인 Status line에서 가장 중요한 부분이 3자리 상태 코드다. 이 코드 하나로 요청이 성공했는지, 클라이언트 오류인지, 서버 오류인지가 판별된다.
...
"상태 줄은 요청 성공 또는 실패를 전달하는 세 자리 상태 코드를 포함한다. 2XX 코드는 성공을, 4XX와 5XX 코드는 오류를 나타낸다. 3XX 코드는 URL 리다이렉션을 나타낸다."

- **status line**: HTTP 응답의 첫 줄. `HTTP/1.1 200 OK` 형태 — HTTP 버전, 상태 코드, 상태 텍스트.

```
상태 코드 4클래스

1XX   정보 (Information)    거의 보이지 않음
2XX   성공 (Success)         ← 정상 처리
...
자주 보는 코드:
- **200 OK**: 범용 성공 응답. GET 성공 시 일반적으로 사용.
- **201 Created**: POST 성공 — 새 리소스가 생성됨.
- **400 Bad Request**: 서버가 처리할 수 없는 잘못된 요청.
- **404 Not Found**: 리소스가 존재하지 않음.

추가로 알아두면 좋은 코드:
- **401 Unauthorized**: 인증 실패 (토큰 없음/만료)
- **403 Forbidden**: 인가 실패 (토큰은 있지만 권한 없음)
...
상태 코드 4개 클래스가 면접 단골 질문이다 — 2XX 성공, 3XX 리다이렉션, 4XX 클라이언트 오류, 5XX 서버 오류. DevTools Network 탭에서 상태 코드를 보는 것이 API 디버깅의 첫 번째 단계다. 4XX이면 내 요청(URL, 헤더, 바디)을 점검하고, 5XX이면 서버 로그를 봐야 한다.
