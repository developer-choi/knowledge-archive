...
보통 하나의 프로세스를 메인 프로그램과 연결하고, 자식 프로세스를 그로부터 파생된 병렬 프로세스와 연결한다.
...
Unix/Linux 계열 OS에서 프로세스 생성과 종료는 `fork`, `exec`, `wait`, `exit` 네 가지 시스템 콜로 이루어집니다. Node.js의 `child_process`는 이 위에 추상화된 API입니다.
...
- **`fork()`**: 부모 프로세스의 복사본으로 자식 프로세스를 생성합니다. 자식은 부모의 코드·데이터를 처음에 공유(Copy-on-Write)하다가 이후 분리됩니다.
- **`exec()`**: 자식 프로세스 메모리에 새 프로그램을 덮어쓰고 실행을 시작합니다. `fork` 직후 자식은 부모와 동일한 코드를 실행하므로, 다른 프로그램을 실행시키려면 `exec`으로 교체합니다.
- **`wait()`**: 부모가 자식의 종료를 기다립니다. 자식이 종료 코드를 반환할 때까지 부모를 블록.
- **`exit()`**: 자식이 종료됩니다. OS에게 할당받은 자원을 반납하고 OS가 자원을 해제합니다.

`abort()`로 강제 종료하는 예외 케이스:
- 자식이 할당된 자원 사용량을 초과한 경우
- 자식 task가 더 이상 필요하지 않은 경우
- 부모가 먼저 종료되어 자식만 남은 경우

---

## 종합

흐름 순서: `fork()` → `exec()` → (자식 실행) → `exit()` → 부모의 `wait()` 반환 → 부모 재개.

Node.js 대응:
- `child_process.fork()` = Unix의 `fork` + `exec`를 합친 고수준 API
- `child.kill()` = `abort()` (강제 종료)
- `child.on('exit', ...)` = `wait()` (종료 감지)
...
OS가 수십~수백 개의 프로세스를 동시에 관리하려면 각 프로세스를 고유하게 식별할 방법이 필요합니다. 그 방법이 PID(Process ID)입니다.

---

## 본문

- **PID(Process ID)**: 각 프로세스에 부여되는 고유 정수 번호. 프로세스가 생성될 때마다 증가하는 값이 배정됩니다. Task Manager(Windows) 또는 `ps` 명령(Linux)에서 확인 가능합니다.
- **PID 1 — systemd**: 리눅스 기준으로 부팅 직후 가장 먼저 실행되는 프로세스는 `systemd`로 PID 1을 받습니다. `systemd`는 다른 모든 프로세스의 조상입니다.
- **systemd가 생성하는 대표 프로세스 예시**:
  - `logind`: 시스템에 직접 로그인하는 클라이언트 관리
  - `sshd`: SSH 프로토콜로 접속하는 클라이언트 관리
  - RDP 관련 프로세스: 윈도우 원격 접속 관리

---

## 종합

PID는 "외울 필요 없는 값"이지만, "모든 프로세스에는 고유 번호가 있고 OS는 이 번호로 관리한다"는 구조는 중요합니다.
