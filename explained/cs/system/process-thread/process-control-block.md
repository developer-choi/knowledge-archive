...
결국 process state에 저장하는 것은 **CPU 레지스터 전부**다. 문장에서 쓴 용어가 길어 보이지만 아까 계층도와 1:1 매핑된다.

- **general-purpose CPU registers → General Purpose Registers**: 계산 중간값이 여기 있음. 덮어씌워지면 연산을 처음부터 다시 해야 함
- **CPU process status word → Status Register**: CPU가 방금 한 연산의 특성(결과가 0인지, 음수인지 등)을 플래그로 기록하는 레지스터. `if` 같은 조건 분기가 이 플래그를 읽어 동작하므로, 저장 안 하면 재개 시 분기가 엉뚱하게 이어짐
- **stack and frame pointers → Stack & Frame Pointers**: 콜 스택 위치와 현재 함수 프레임 기준점. 없으면 어떤 함수 안에 있었는지 몰라 스택 전체가 망가짐

#### "word"라는 표현에 대해

컴퓨터 구조에서 word = CPU가 한 번에 처리하는 데이터 단위(32비트면 32비트, 64비트면 64비트). 초기 IBM 메인프레임 시절 상태 플래그들을 레지스터 하나(= 1 word)에 몰아넣고 "Program Status Word(PSW)"라고 부른 게 굳어진 표현이다. 지금은 rflags 하나가 그 역할을 한다.
