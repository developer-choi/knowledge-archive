...
혼동 포인트: "program code"(구성요소 질문)와 "텍스트 영역"(메모리 영역 질문)은 다른 관점에서 같은 것을 가리킵니다. "무엇이 있나" → program code. "메모리 안 어디에 있나" → 텍스트 영역.
...
멀티태스킹 환경에서 OS는 수십 개 프로세스를 동시에 관리합니다. "지금 어떤 프로세스가 CPU를 기다리는가?", "어느 프로세스에 CPU를 줄 것인가?" — 이것을 결정하고 추적하려면 각 프로세스의 상태와 스케줄링 정보가 PCB에 있어야 합니다.

---

## 본문

여러 프로세스를 동시에 관리할 때 각 PCB에 저장되는 항목:

- **프로세스 상태 필드**: created / waiting / running / blocked / terminated 중 현재 상태. 스케줄러가 다음 CPU 수혜자를 고를 때 waiting 상태인 것만 후보가 됩니다.
- **스케줄링 우선순위**: 어떤 프로세스에게 CPU를 먼저 줄지. 리눅스 nice 값처럼 숫자로 표현되며, 우선순위가 높은 프로세스가 CPU를 더 자주 받습니다.
- **준비 큐(ready queue) 연결 정보**: waiting 상태인 프로세스들을 OS가 큐로 관리합니다. 각 PCB가 큐 안에서 앞·뒤 노드를 가리켜 연결 리스트를 형성합니다.
- **I/O 대기 정보**: blocked 상태일 때 무엇을 기다리는지. 기다리던 자원이 도착하면 OS가 이 정보를 보고 해당 프로세스를 waiting으로 전환합니다.

---

## 종합

"공정한 CPU 배분"이 OS 멀티태스킹의 핵심입니다. 그러려면 각 프로세스가 "지금 어떤 상태이고(상태 필드), 얼마나 기다렸고(우선순위), 무엇을 기다리는지(I/O 대기 정보)"를 PCB에 항상 최신으로 유지해야 합니다.

앞 질문의 구성요소 분류로 대응하면: 이 정보들이 **coordinate** 역할 — "여러 프로세스의 동시 실행을 조율하기 위한 자료구조"에 해당합니다.
...
- **context switches**: 한 프로세스에서 다른 프로세스로 CPU 점유를 전환하는 작업. OS가 CPU 하드웨어 안의 값들을 RAM의 PCB로 복사해두고, 다음 프로세스의 PCB 값을 CPU에 얹어줍니다. 저장되는 것은 두 가지입니다: **Program Counter**(특수 목적 레지스터 — 어디서 이어받을지, 명령어 주소)와 **범용 레지스터(General-Purpose Register)**(for문 `i`처럼 계산 중이던 값들). PC는 "책갈피(몇 페이지)", 범용 레지스터는 "읽다 기억해둔 내용"으로 구분하면 됩니다.
...
시분할 멀티태스킹의 타임라인 (1코어 기준):

```
   시간 ─────────────────────────────────────────────>
        0ms   10ms  20ms  30ms  40ms  50ms  60ms

   CPU: |AAAA|BBBB|CCCC|AAAA|BBBB|CCCC|AAAA|
        |    |    |    |    |    |    |
        v    v    v    v    v    v    v
       ctx  ctx  ctx  ctx  ctx  ctx  ctx
       sw   sw   sw   sw   sw   sw   sw

   Process A: 실행 ─── 대기 ─── 실행 ─── 대기 ─── 실행 ─── 대기
   Process B: 대기 ─── 실행 ─── 대기 ─── 실행 ─── 대기 ─── 실행
   Process C: 대기 ─── 대기 ─── 실행 ─── 대기 ─── 대기 ─── 실행

   사용자 인지: A, B, C 모두 "동시에" 실행 중 (concurrency)
   실제: 한 순간에 1개만 실행 (CPU 1코어 기준)
```

```
   Concurrency vs Parallelism

   Concurrency (1코어):  |A|B|C|A|B|C|A|B|C|   "동시처럼 보임"
                          시분할 + 빠른 전환

   Parallelism (4코어):  Core1: AAAAAAAAAA
                         Core2: BBBBBBBBBB    "물리적으로 동시"
                         Core3: CCCCCCCCCC
                         Core4: DDDDDDDDDD
```

핵심: 1코어에서의 멀티태스킹은 본질적으로 빠른 시분할이고, "동시 실행 중"은 환상이다. 진짜 병렬은 코어 수만큼만 가능.
...
```
   ...
   - 자식은 fork 시점의 부모 메모리를 복사 (Copy-on-Write)
   - 자식은 부모 권한의 부분집합만 가능
...
Chrome, VS Code, 터미널을 동시에 켜두면 OS는 이것들을 어떻게 구분할까요? 이름으로는 충분하지 않습니다 — Chrome 탭마다 별도 프로세스가 뜨면 이름이 전부 `chrome`입니다. OS는 고유 번호로 구분합니다.

---

## 본문

OS는 각 프로세스에 **PID(Process ID)** 라는 고유 번호를 부여합니다.

- 새 프로세스가 생성될 때마다 증가하는 정수 값을 받습니다. 부팅 이후 단조 증가하므로 현재 실행 중인 프로세스들은 모두 서로 다른 PID를 가집니다.
- **PID 1**: 리눅스 기준으로 부팅 직후 첫 프로세스는 항상 Systemd(또는 init)입니다. 이후 모든 프로세스는 Systemd에서 파생됩니다.
- Systemd가 생성하는 대표적인 프로세스들:
  - `logind`: 로컬 로그인 세션 관리
  - `sshd`: SSH 원격 접속 클라이언트 관리
  - `rdp`: 윈도우 원격 접속 관리
- 리눅스 터미널에서 `ps aux` 또는 `ps -ef`로 모든 프로세스의 PID를 확인할 수 있습니다.

---

## 종합

PID는 OS가 프로세스를 관리하는 모든 시스템 콜의 기준이 됩니다. `kill -9 1234`처럼 종료도 PID로, 자식 프로세스 추적도 부모 PID로. 번호 자체를 외울 필요는 없고, "OS가 각 프로세스에 부여하는 고유 식별자"라는 개념이 핵심입니다.
...
`node app.js`가 내부적으로 `ffmpeg`를 실행해서 동영상을 인코딩한다고 해봅시다. Node 프로세스(부모)가 ffmpeg 프로세스(자식)를 만들고, 끝나면 정리하는 흐름 전체가 시스템 콜로 구현됩니다. Linux 기준의 표준 흐름입니다.
...
- **`fork()`**: 부모 프로세스를 복제해 자식 프로세스를 만듭니다. 자식은 부모의 메모리 상태를 복사받고 같은 코드를 이어서 실행합니다. `fork()` 반환값으로 "나는 부모다(자식 PID 반환)"와 "나는 자식이다(0 반환)"를 구분합니다.
- **`exec()`**: 자식에서 호출해 새로운 프로그램을 메모리에 적재합니다. `fork()`로 복사된 부모 코드를 버리고 ffmpeg 코드로 교체하는 것. 이후 자식은 ffmpeg 프로세스로 동작합니다.
- **`wait()`**: 부모가 호출해 자식의 종료를 기다립니다. 자식이 종료되면 `wait()`이 반환되고 부모는 자식의 종료 코드를 받습니다. 이걸 안 하면 종료된 자식 PCB가 RAM에 남아 좀비 프로세스가 됩니다.
- **`exit()`**: 자식이 작업을 마치고 스스로 종료할 때 호출합니다. OS에 할당받은 자원(메모리, 파일 핸들 등)을 반납합니다.

예외: **`abort()`** 로 부모가 자식을 강제 종료시키는 경우도 있습니다 — 자식이 할당 자원을 초과하거나, 더 이상 필요 없어지거나, 부모가 먼저 종료되는 경우.

---

## 종합

fork → exec → (자식 작업) → exit, 부모는 wait → 자식 종료 확인의 순서입니다.

```
부모 프로세스                     자식 프로세스
────────────────                  ────────────────
fork() 호출       ──복제──→       fork() 반환 (0)
                                  exec() — ffmpeg로 교체
                                  ... 작업 ...
wait() 대기       ←──종료──       exit() 호출
자식 종료 코드 수령
...
Node.js에서 `child_process.spawn('ffmpeg', ...)` 를 호출하면 내부적으로 이 흐름이 그대로 일어납니다.
