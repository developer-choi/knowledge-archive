...
- **control** (통제): 실행 중에 CPU를 빼고 돌려주는 순간 — for문 1억 번 돌던 프로세스에서 OS가 CPU를 빼앗을 때, 나중에 재개하려면 중단 시점을 저장해둬야 합니다. 저장되는 것은 두 가지: **Program Counter**(어디서 이어받을지 — 명령어 주소)와 **레지스터 상태**(계산 중이던 값들, `i = 37281842` 같은 것). CPU 레지스터는 CPU 하드웨어 안에 있고 PCB는 RAM에 있습니다. 컨텍스트 스위치 = CPU 레지스터 값을 RAM의 PCB로 복사하는 것, 재개 = PCB 값을 다시 CPU 레지스터에 얹는 것.
