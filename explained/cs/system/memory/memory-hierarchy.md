...
Register → L1 → L2 → L3 → RAM → Disk → Remote 순으로 계층이 구성된다. 위로 갈수록 빠르고 작고 비싸며, 아래로 갈수록 느리고 크고 저렴하다.
...
영어로는 "Storage"라는 표현을 쓴다. "Memory"는 RAM을 가리키는 경우가 많아 혼동을 피하기 위해 보조기억장치에는 Storage를 선호한다.
