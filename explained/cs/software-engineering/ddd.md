...
```
       ...
       +---------------+---------------+
       |               |               |
   Auth Context   Checkout Context  Shipping Context
   ...
   +-------+       | Money |       +-------+
                   +-------+
   각 컨텍스트의 "User"는 별개 모델
   (auth.User != checkout.User)
```

핵심: 단일 거대 모델 대신 컨텍스트별 독립 모델. 같은 단어("User", "Order")라도 컨텍스트가 다르면 별개 타입.
...
```
   ...
   +-----------------------------------+
              ^
              |  X: 외부에서 내부 객체 직접 참조 금지
              |     (item.quantity = 999 같은 우회)
   외부 코드 ─/
```

핵심: 모든 변경은 루트의 메서드를 통과 → 루트가 일관성 검사 수행. 내부 객체 직접 참조를 막아야 이 검사가 의미 있음.
