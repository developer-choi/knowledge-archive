...
User Annotation이 짚듯, 운영자가 내 비밀번호를 몰라도 이 값이 내 비밀번호인지는 알 수 있다 — 입력한 값의 해시값과 저장된 해시값을 비교하면 되니까. 역변환은 매우 어렵다 (단방향성).
...
User Annotation이 짚듯, 해시 함수를 쓴 뒤에도 추가 절차(mod)가 있다. 해시 함수는 해시의 일부다.
...
User Annotation이 짚는 중요한 뉘앙스: "search"는 실제로 "access(접근)"에 가깝다.
