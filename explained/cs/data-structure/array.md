...
User Annotation이 짚듯, `[1,2,3,4]`에서 `[2,3]`은 subarray지만 `[1,3]`은 아니다 — 2와 4를 건너뛰었기 때문이다. `[1,3]`은 subsequence다.
...
AI Annotation이 짚듯, 이것이 가능한 이유는 두 전제 조건이 동시에 충족되기 때문이다:
1. base address를 알고 있다 (배열 생성 시 고정)
2. 모든 원소의 크기가 동일하다 (같은 타입이므로)

User Annotation이 짚듯, Random의 반대는 Sequential Access다. Linked List에서 3번째 노드를 찾으려면 head → 1번 → 2번 → 3번 순서로 포인터를 세 번 따라가야 한다 — 인덱스가 커질수록 시간이 선형으로 늘어난다.
...
User Annotation이 짚듯, 연속된 메모리에 저장되어 있으므로 CPU 캐시가 인접 데이터를 미리 가져올 수 있어 캐시 히트율이 높아진다.
...
User Annotation이 짚듯, 삭제는 반대 방향 — 원소를 지우고 뒤의 원소들을 한 칸씩 앞으로 당긴다. 배열엔 공백이 있으면 안 되기 때문이다.
...
User Annotation이 짚듯, 삭제도 동일한 패턴이지만 반대 방향 — 맨 앞을 삭제하면 나머지 전체를 한 칸씩 앞으로 당긴다.
