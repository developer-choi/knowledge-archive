...
자식이 1개인 노드가 없으면 된다. 상위 레벨에서 자식이 없는 노드(리프 노드)가 있는 경우도 Full Binary Tree 조건을 만족한다.
...
배열은 주로 Heap이나 Complete Binary Tree처럼 구조가 꽉 찬 트리에서 압도적으로 유리하다.
