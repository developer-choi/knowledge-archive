...
**구분자 중복**
- `---`가 빈 줄을 사이에 두고 2개 이상 연속하면 위반. 코드 블록(` ``` `) 내부의 `---`는 제외하고 판단한다. ([document-structure.md](../../contexts/document-structure.md)의 "explained/ 파일 구조" 참고)
...
| 구분자 중복 | 연속 `---` 제거하여 1개로 통합 (코드 블록 내부 제외) |
