...
- **OA 언어**: Official Answer 본문이 공식 문서 원문(대체로 영어)으로 유지되는가. (린터가 OA 본문의 한글을 위치 불문 전부 잡으므로, 남는 판정은 "한글은 없는데 영어 자체가 원문이 아니라 AI가 지어낸 영어인가"뿐이다.)
...
**아래는 검사 지시가 아니라 판정 기록이다.** `/validate`가 매번 훑을 목록이 아니라, "이 규칙은 왜 검사기가 없는가"를 적어둔 대장이다 — 위 「판단 체크」의 항목들이 실제 검사 대상인 것과 성격이 다르다.
...
| 출처 순위 — 상위 출처에서 찾으면 하위로 안 내려간다 | primary-source/SKILL.md | 같은 이유. 어느 순위에서 멈췄는지가 파일에 안 남는다 | 2026-08-05 |
| production-guide Before/After 실행 여부 | production-guide.md | 절차를 밟았는지 자체가 산출물에 안 남는다. 결과물 정합은 린터가 따로 본다 | 2026-08-05 |
| 폴더 선택·파일명 키워드 적절성 | file-placement §1·§2 | "핵심 키워드가 들어갔나", "주제가 맞나"는 의미 판단이다. 소문자·하이픈 같은 형식만 린터가 본다 | 2026-08-05 |
| 목차 들여쓰기와 본문 위계의 논리적 일치 | document-structure「계층 구조 표현」 | 순서 1:1은 목차-본문 순서 검사가 보지만, 꼬리질문이 논리적으로 부모에 종속되는지는 뜻으로 갈린다 | 2026-08-05 |
...
| 위반 | 조치 |
|------|------|
| 커버리지 누락 (explained 존재 + 일부 질문 누락) | 자동 수정 불가 — `/digest`로 해당 파일을 다뤄 해설 생성. 누락 목록만 보고. **knowledge 질문을 지워서 맞추지 않는다.** |
| 짝 부재 (explained 파일 자체가 없음) | 자동 수정 불가 — `/digest`로 해설 생성. 목록만 보고. |
| 고아 섹션 | 사용자 확인 후 섹션 삭제 |
| 고아 파일 | 사용자 확인 후 파일 삭제 |
| 질문 순서 불일치 | explained 섹션 순서를 knowledge 질문 순서에 맞춰 재배치 (knowledge가 기준) |
| explained 본문에 원문 인용 없음 | 자동 수정 불가 — 원문을 어느 조각으로 끊어 인용하고 그 위에 해설을 어떻게 다시 얹을지가 해설 다시 쓰기라 `/digest`의 몫이다. 목록만 보고. |
| OA 본문 수정 + explained 미동기화 | 자동 수정 불가 — 바뀐 OA를 해설에 어떻게 반영할지가 해설 쓰기라 `/digest`의 몫이다. **explained가 반드시 따라가야 하는 것은 아니다** — OA에서 덜어낸 문장이 해설에는 그대로 남아도 되고, 반대로 해설에만 있는 보충 인용도 정상이다([explanation-guide.md](../../contexts/explanation-guide.md) §6). 그 질문의 해설을 한 번 읽고 판단한 뒤, 손댈 것이 없으면 그대로 둔다. |
| 본편 없는 `.sub.md` | 자동 수정 불가 — 본편이 어디로 갔는지 git log로 확인 후 보고. 본편이 **이동·개명**됐으면 곁가지를 따라 옮기고, 본편이 **삭제**됐으면 곁가지를 본편 이름으로 개명한다(`.sub` 제거, explained 미러도 함께). 어느 쪽인지는 사용자가 판단 ([file-placement.md](../../contexts/file-placement.md)「곁가지 분리」). |
