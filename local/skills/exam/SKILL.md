...
페이로드 형태: `{ __skill: "ka-exam", ts: <ms>, sum: "<답변 지문>", answers: [<Q1 답변>, <Q2 답변>, ...] }`. `answers` 배열의 인덱스가 문항 순서(0-based)와 대응한다. `sum`은 시험지가 제출 시점에 답변들로 찍은 지문이다.
...
- **붙여넣은 것을 통째로 한 번 옮긴다.** `sum`을 포함해 전부 그대로 옮긴다. 답변을 문항별로 나눠 적거나, 요약·정리·줄바꿈 정돈을 하지 않는다.
- 여기서 답변에 손대면 Phase 4에서 렌더러가 `sum`을 다시 찍어 대조하다가 어긋난 값을 보고 죽는다. 오타·순서 바뀜·한 칸 누락 전부 이 자리에서 걸리므로, 걸리면 붙여넣은 원문에서 다시 옮긴다.
