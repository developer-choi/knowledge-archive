...
`knowledge/<rel>.md`와 1:1 대응하는 해설 캐시. 복습 때 직접 읽는 산출물이다.
...
옮긴 뒤 `npm run validate-lint`를 돌린다. 본문의 상대 링크가 실재하는 파일을 가리키는지, 앵커가 붙었으면 대상에 그 제목의 헤딩이 있는지까지 본다 — 파일만 옮기고 링크를 안 고치면 여기서 걸린다.
