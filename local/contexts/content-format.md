...
| `google-doc` | 구글 문서 등 외부 필기에서 변환된 파일 |
...
| 값 | 의미 |
|----|------|
| `1` | 기술면접 필수 대비 — 어떤 기술면접이든 무조건 준비하는 기저 항목 |
| `2` | 공고에서 해당 주제를 추가로 요구·언급할 때만 준비하는 항목. 언급이 없으면 그 주제는 아주 흔한 기본만 다루고 깊게는 준비하지 않는다 (예: 공고에 "JS 기초 중요" 명시 시 JS를 딥하게 준비) |
| `5` | 참고용 (나중에 찾아볼 수도 있어서 보관) |
| (빈 값) | 미분류 |

- 3~4 정의는 미확정. 미확정 단계에서는 비워둔다.
- **AI는 priority를 추측해 채우지 않는다.** 면접 대비 우선순위는 사용자가 학습 전략에 따라 직접 정하는 값이라, AI가 "면접 빈출이니 1" 식으로 판단해 쓸 자리가 아니다. 사용자가 대화에서 정해준 값을 AI가 대신 적는 경로는 열려 있다.
- 빈 키(`priority:`)를 새로 심지도 않는다. 사용자가 우선순위를 정할 때 직접 키를 추가한다.

---

### publishable

이 문서를 외부 채널 노출 후보로 삼을지. `false`면 다른 조건과 무관하게 후보에서 빠진다. 선택 키이며, 없으면 미분류로 다른 조건에 따라 판정된다.

값은 `true`·`false`만 쓴다. 후보를 뽑는 쪽이 불리언으로만 읽으므로 `no`·`No` 같은 표기는 조용히 무시되고, 빼려던 문서가 그대로 후보에 남는다. 키가 아예 없는 것과 구분이 안 돼 아무도 못 알아채므로 린터가 어휘를 본다.
...
#### 절대 원칙
- 질문은 반드시 **답변 섹션(Official Answer, Additional Answer, User Answer)의 내용으로 답할 수 있어야** 한다.
- Official Answer가 확보 가능하면 반드시 포함한다. 아직 공식 출처를 찾지 못한 경우 Additional/User Answer만으로 구성할 수 있다.
...
| `### Additional Answer` | 공식 문서 아님 (AI·사용자 작성) | 공식 출처 미확보 or OA에 없는 보충 — 면접 답변 수준 암기 필요 |
| `### User Answer` | 사용자 작성 | 사용자의 해석, 경험 기반 답변 |

### 작성 원칙
- **빈 섹션 금지**: 채울 본문이 없는 섹션 헤딩(`### Official Answer`, `### Additional Answer`, `### User Answer`, `### Reference`, 그리고 아래 §5의 `### Review Note`·`### Frequent Mistakes`)은 만들지 않는다. 본문이 생기는 시점에 섹션을 추가한다.
...
- **원문 보존**: 공식 문서 원문은 절대 수정하지 않는다. 마이그·재편 시에도 영어 원문(Official 콘텐츠)을 삭제해서는 안 된다 — 형태가 바뀌더라도(OA 단락 통합 등) 원문 자체는 반드시 어딘가에 보존되어야 한다.
  - **예외 — 출처 문서가 개정된 경우**: 출처가 새 버전으로 바뀌어 그 원문이 더 이상 현행이 아니면 삭제할 수 있다. 이 규칙은 정리·재배치 중에 원문을 잃지 말라는 뜻이지, 낡은 버전을 영구 보관하라는 뜻이 아니다. 지운 내용은 git 이력에 남는다. `/digest`가 「이번에 읽는 문서가 기준이다」로 이 경우를 다룬다.
- **AI의 OA 신규 생성 금지**: Official Answer는 공식 문서에 실재하는 원문을 옮긴 것만 담는다. AI가 Wikipedia·MDN 등을 WebFetch/WebSearch로 검색해 OA를 새로 구성하지 않는다. 영어 원문이 확보되지 않으면 OA를 만들지 않고, 해당 내용은 User/Additional Answer로 작성한다. (digest 등 Q&A를 생성하는 스킬이 따르는 원칙.)
- **OA 한글 금지**: OA 본문에는 한글을 한 글자도 넣지 않는다. 위치를 가리지 않는다 — 헤딩 바로 뒤 도입 문장이든, 원문 단락 사이 연결 문장이든, 영어 문장 안에 끼운 괄호 주석이든 전부 해당한다. `#### H4` 소제목은 원문이 아니라 구조 표시이므로 예외다.
  - **왜 예외를 안 두는가**: OA에 한글이 허용되면 "이게 원문인가 내가 쓴 것인가"를 매번 사람이 판단해야 한다. 한글이 0이면 그 판단이 사라지고 린터가 결정론적으로 잡는다. 예외를 하나 열면 판단이 되살아나 그 보장이 깨진다.
  - **영어로 보충하는 것도 안 된다**: `To solve this [SSR limitation]` 처럼 대괄호로 끼워 넣는 방식은 한글보다 나쁘다. 원문에 이미 대괄호가 쓰이므로(마크다운 링크, `the [Symbol.iterator]() method` 같은 원문 표기) 보충분과 원문을 구분할 방법이 아예 없다.
  - **그럼 보충이 필요하면**: ⓐ 선행 문맥도 원문에서 그대로 더 가져오거나, ⓑ 보충 설명을 `### Additional Answer`·`### User Answer`에 쓰거나(거기는 한글 자리다), ⓒ 출처가 다른 내용이면 질문을 나눈다. OA 밖으로 내보내는 것이지 OA 안에서 표기를 궁리하지 않는다.
