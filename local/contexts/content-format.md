...
- **AI의 OA 신규 생성 금지**: Official Answer는 공식 문서에 실재하는 원문을 옮긴 것만 담는다. AI가 Wikipedia·MDN 등을 WebFetch/WebSearch로 검색해 OA를 새로 구성하지 않는다. 영어 원문이 확보되지 않으면 OA를 만들지 않고, 해당 내용은 User/Additional Answer로 작성한다. (digest·convert 등 Q&A를 생성하는 스킬이 공통으로 따르는 원칙.)
- **OA 한글 금지**: OA 본문에는 한글을 한 글자도 넣지 않는다. 위치를 가리지 않는다 — 헤딩 바로 뒤 도입 문장이든, 원문 단락 사이 연결 문장이든, 영어 문장 안에 끼운 괄호 주석이든 전부 해당한다. `#### H4` 소제목은 원문이 아니라 구조 표시이므로 예외다.
  - **왜 예외를 안 두는가**: OA에 한글이 허용되면 "이게 원문인가 내가 쓴 것인가"를 매번 사람이 판단해야 한다. 한글이 0이면 그 판단이 사라지고 린터가 결정론적으로 잡는다(K5). 예외를 하나 열면 판단이 되살아나 그 보장이 깨진다.
