...
- **AI는 priority를 추측해 채우지 않는다.** 면접 대비 우선순위는 사용자가 학습 전략에 따라 직접 정하는 값이라, AI가 "면접 빈출이니 1" 식으로 판단해 쓸 자리가 아니다. 사용자가 대화에서 정해준 값을 AI가 대신 적는 경로는 열려 있고, 그때는 승인 다이얼로그에서 사용자가 판정한다.
