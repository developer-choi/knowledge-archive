// knowledge frontmatter의 `priority`를 AI가 새로 쓰거나 바꾸는 것을 막는 PreToolUse 훅.
//
...
export function violation(before, after) {
  ...
  if (wasSet === null || wasSet === "") {
    return `knowledge frontmatter에 \`priority: ${willBe}\`를 새로 쓰려 합니다. priority는 사용자가 학습 전략에 따라 직접 정하는 값이라 AI가 채우지 않습니다 (content-format.md §1「priority」). 해당 값을 빼고 다시 쓰세요.`;
  }
  if (wasSet !== willBe) {
    return `knowledge frontmatter의 priority를 "${wasSet}" → "${willBe}"로 바꾸려 합니다. 사용자가 정해둔 값은 그대로 보존합니다 (content-format.md §1「priority」).`;
  }
  return null;
}
