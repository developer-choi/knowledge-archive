...
# Questions
- [문자열(String)이란 무엇인가?](#문자열string이란-무엇인가)
- [문자열이 불변(Immutable)이라는 것은 무엇을 의미하는가?](#문자열이-불변immutable이라는-것은-무엇을-의미하는가)
  - [문자열이 불변인 것이 왜 중요한가?](#문자열이-불변인-것이-왜-중요한가)
- [문자열은 메모리에 어떻게 저장되는가?](#문자열은-메모리에-어떻게-저장되는가)
- [문자열의 길이를 구하는 시간 복잡도는?](#문자열의-길이를-구하는-시간-복잡도는)
- [두 문자열의 동일성 비교(equality check)의 시간 복잡도는?](#두-문자열의-동일성-비교equality-check의-시간-복잡도는)
