...
# Questions
- [배열이란 무엇인가?](#배열이란-무엇인가)
  - [부분 배열(Subarray)이란 무엇인가?](#부분-배열subarray이란-무엇인가)
- [배열이 왜 필요한가?](#배열이-왜-필요한가)
- [배열의 장점은 무엇인가?](#배열의-장점은-무엇인가)
  - [임의 접근(Random Access)이란 무엇이고, 왜 O(1)인가?](#임의-접근random-access이란-무엇이고-왜-o1인가)
  - [배열이 캐시 친화적인 이유는?](#배열이-캐시-친화적인-이유는)
- [배열의 단점은 무엇인가?](#배열의-단점은-무엇인가)
  - [배열에서 삽입/삭제가 O(n)인 이유는?](#배열에서-삽입삭제가-on인-이유는)
    - [삽입 위치가 성능에 어떤 영향을 미치는가?](#삽입-위치가-성능에-어떤-영향을-미치는가)
